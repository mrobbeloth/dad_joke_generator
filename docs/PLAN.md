...
- [ ] MS03: Billing alert configured at the cost threshold recorded in `/dadjokes/cost_alarm_threshold_usd` _(updated 2026-05-29: OSU OTDI Cloud Platform (Lok Yu) confirmed the active org-level cost-allocation tag is `Proj`, not `Project`. Both modules' `provider.tf` `default_tags` now emit `Proj=dadjokes` alongside the existing `Project=dadjokes`. All 30 taggable resources re-tagged. Awaiting ~24h cost-data backfill on the `Proj` tag before flipping `var.budget_enabled = true` and re-applying. Verification probe: `aws ce get-cost-and-usage --filter '{"Tags":{"Key":"Proj","Values":["dadjokes"]}}'` should return non-zero for the current month after backfill. Tick MS03 once the budget is provisioned and the operator has confirmed the AWS Budgets console reflects real dadjokes-only spend.)_

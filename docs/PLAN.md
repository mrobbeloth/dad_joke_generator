...
- [x] MS03: Billing alert configured — dadjokes-scoped AWS Budget, $20/month (2026-05-29) _(Implemented as an AWS Budget filtered to `user:Proj$dadjokes` (infra/terraform-bootstrap/budgets.tf), NOT a CloudWatch billing alarm. The original account-wide `AWS/Billing EstimatedCharges` CloudWatch alarm was removed 2026-05-29 because that metric cannot be scoped to a cost-allocation tag and fired constantly on other OSU projects' spend on this shared account. The `Proj` cost-allocation tag (active at the org payer level per OSU IT) is emitted on every resource via provider default_tags. Budget notifies robbeloth.1@osu.edu at 50% forecasted / 80% / 100% of $20. Cost-data backfill on the `Proj` tag may take up to ~24h; the budget under-reports until then but never over-alarms.)_
...
- _(Backlog)_
  ...
  - `r163-design-doc-drift`: `design.md` R16.3 still describes the cost guardrail as a CloudWatch `AWS/Billing EstimatedCharges` alarm, but on 2026-05-29 it moved to a dadjokes-scoped AWS Budget (`infra/terraform-bootstrap/budgets.tf`, `user:Proj$dadjokes`, $20/mo) because billing metrics can't be tag-scoped on the shared OSU account. Update R16.3 (and any Property 31 wording that assumes a CloudWatch cost alarm feeds `dispatch_cost_alert`); evaluate whether `/dadjokes/cost_alarm_threshold_usd` + `var.cost_alarm_threshold_usd` are now vestigial. Project board item `PVTI_lAHOAG93-M4BYamIzgxzD24`. Added 2026-05-29.
